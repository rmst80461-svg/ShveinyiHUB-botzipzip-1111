...
def _get_web_admin_orders_url() -> str:
    ...
    web_admin_url = os.getenv("WEB_ADMIN_URL") or (
...
def get_admin_ids() -> List[int]:
    ...
    try:
        ...
        logger.debug("get_admins не доступна или завершилась с ошибкой")
    return ids
...
def is_user_admin(user_id: int) -> bool:
    ...
    try:
        ...
        logger.error(f"Ошибка проверки статуса администратора в БД для {user_id}: {e}")
        return False
...
async def admin_spam(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        logger.exception("Ошибка при получении журнала спама")
        if update.effective_message:
...
async def broadcast_preview(
    ...
    await update.effective_message.reply_text(
        preview_text, reply_markup=keyboard, parse_mode="Markdown"
    )
...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Унифицированная точка входа для callback админ-панели"""
...
async def admin_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data in ["📊 Все заказы", "admin_orders_all"]:
        await admin_orders(update, context)
        return

    if data in ["📈 Статистика", "admin_stats"]:
        await admin_stats(update, context)
        return

    if data in ["👥 Пользователи", "admin_clients"]:
        await admin_users(update, context)
        return

    if data in ["📢 Рассылка", "broadcast_menu"]:
    ...
    if data == "open_web_admin":
        ...
        await query.edit_message_text(
            f"🌐 Веб-панель: {url}", reply_markup=keyboard, parse_mode="Markdown"
        )
        return
    ...
    if data in status_map:
        ...
        try:
                        ...
                        [
                            [
                                InlineKeyboardButton(
                                    "◀️ Назад", callback_data="admin_back_menu"
                                )
                            ]
                        ]
                    ),
            ...
            for order in orders[:20]:
                ...
                service_display = SERVICE_NAMES.get(
                    order.service_type, order.service_type or "—"
                )
                text += (
                    f"📦 {formatted} — {order.client_name or 'Аноним'}\n"
                    f"🛠 _{service_display}_\n"
                    f"📞 {phone}\n\n"
                )
                keyboard.append(
                    [
        ...
        try:
            ...
            if delete_order(order_id):
                ...
                await query.message.edit_text(
                    f"🗑 Заказ #{order_id} был удален из базы данных."
                )
            else:
...
async def admin_view_order(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    phone_display = (
        order.client_phone
        if order.client_phone and order.client_phone != "Telegram"
        else "📲 Telegram"
    )
    ...
    try:
        ...
        logger.error(f"Ошибка при отображении заказа в admin_view_order: {e}")
        try:
            await query.edit_message_text(
...
async def change_order_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        formatted = (
            format_order_id(order.id, order.created_at)
            if order
            else f"#{order_id}"
        )
        client_msgs = {
        ...
        if order and getattr(order, "user_id", None):
            try:
                await context.bot.send_message(
                    chat_id=order.user_id, text=client_text
                )
            except Exception:
                logger.warning(
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "◀️ К списку заказов", callback_data="admin_orders_menu"
                    )
                ]
            ]
        )
...
async def contact_client(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    buttons.append(
        [InlineKeyboardButton("◀️ Назад", callback_data=f"admin_view_{order_id}")]
    )
...
def get_admin_menu_keyboard(stats: Optional[dict] = None) -> InlineKeyboardMarkup:
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                f"🆕 Новые ({new_count})", callback_data="admin_orders_new"
            ),
            InlineKeyboardButton(
        ...
        [
            ...
            InlineKeyboardButton(
                f"📤 Выданные ({issued})", callback_data="admin_orders_issued"
            ),
        ],
        [
        ...
        [
            InlineKeyboardButton(
                "🌐 Веб-админка", callback_data="open_web_admin"
            ),
            InlineKeyboardButton("◀️ Назад", callback_data="admin_back_menu"),
        ],
    ]
